...
- 각 빈 패키지에 **`tsconfig.json` extends 1줄** (상대경로 `{ "extends": "../config-ts/base.json" }` — scoped 이름 `@admin-console/config-ts/base.json` 형태는 TypeScript의 exports 해석 제약으로 `.json` 파일에는 불안정. Slice 04 이후 실제 tsc 빌드가 붙으면 재검토) + **`eslint.config.js`에 `export default [{}]` placeholder**
