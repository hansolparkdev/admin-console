...
Stitch "Admin Console Layout" 원본은 헤더 우측에 `The Executive Lens` 브랜드 배지(Manrope bold)를 포함한다. plan 초기엔 "사이드바 중복 방지"를 이유로 삭제했으나, 원본 스크린 의도(사이드바 wordmark `THE LENS`와 헤더 배지의 계층 대비)를 존중해 복원.

- **Given** 헤더(`role="banner"`)가 렌더되었다
- **When** 헤더 DOM 내부 텍스트를 탐색한다
- **Then** "The Executive Lens" 텍스트가 존재한다
- **And** 해당 텍스트는 Manrope(`var(--font-heading)`) 계열 폰트로 렌더된다
