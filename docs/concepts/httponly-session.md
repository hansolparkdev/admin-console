...
- **세션 관리 주체**: Auth.js v5 (`apps/admin/auth.ts`)
...
- **세션 형식**: **JWE (JSON Web Encryption)** — access_token, refresh_token, expires_at 포함. 브라우저가 내용을 읽을 수 없음 (httpOnly + 암호화 이중 보호)
- **세션 TTL**: `maxAge: 60 * 60 * 24 * 14` (14일 슬라이딩 세션) — `auth.ts`에 고정
- **브라우저 → BFF 전달**: `credentials: "include"` (`apps/admin/src/lib/api.ts`)
- **BFF → 내부 API 변환**: 쿠키에서 session 꺼내 Bearer 토큰으로 변환 (`apps/admin/src/app/api/[...proxy]/route.ts` — refresh-rotation 슬라이스에서 완성)
- **미들웨어 게이트**: `apps/admin/src/middleware.ts` — `auth()` 래퍼로 모든 보호 경로 검사. 세션 없으면 `/login?callbackUrl=…` 307
